...
/// This is similar to a [`Duration`](chrono::Duration), except that it has a fixed start
...
impl NaivePeriod {
    /// Create a new `NaivePeriod` from two [`NaiveDateTime`](chrono::NaiveDateTime) objects.
    ///
    /// # Arguments
    ...
    /// Create a new `NaivePeriod` from a [`NaiveDateTime`](chrono::NaiveDateTime) object and a
    /// [`Duration`](chrono::Duration) object.
    ///
    /// # Arguments
    /// - `start`: A `NaiveDateTime` representing when the `NaivePeriod` will start.
    ...
    /// Retrieve the [Duration](chrono::Duration) this `NaivePeriod` covers.
    #[inline]
